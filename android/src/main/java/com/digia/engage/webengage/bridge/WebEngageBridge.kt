...
internal class WebEngageSdkBridge(
        ...
        private val dataCache: IInAppDataCache = InAppDataCache(),
) : WebEngageBridge {
        ...
        inAppCallback =
                object : InAppNotificationCallbacks {
                    override fun onInAppNotificationPrepared(
                        ...
                        val shouldDispatchToDigia =
                                isDigiaCampaign ||
                                        config.suppressionMode == SuppressionMode.SUPPRESS_ALL
    ...
    // A campaign is Digia-controlled when DigiaInAppContract successfully parsed a contract
    // (indicated by the presence of digiaContractSource after normalizeWithDigiaContract).
    private fun isDigiaCampaign(map: Map<String, Any?>): Boolean {
        return (map["digiaContractSource"] as? String)?.isNotBlank() == true
    }
    ...
    private fun extractInlineEnvelope(data: WECampaignData): InlineCampaignEnvelope? {
        return try {
            ...
            val campaignId =
                    data.campaignId?.takeIf { it.isNotBlank() }
                            ?: root.optString("campaignId").takeIf { it.isNotBlank() }
                                    ?: return null
            val targetViewId =
                    data.targetViewId?.takeIf { it.isNotBlank() }
    ...
    private fun extractInlineMetadata(
        ...
        root.optString("variationId").takeIf { it.isNotBlank() }?.let { put("variationId", it) }
        root.optString("parserType").takeIf { it.isNotBlank() }?.let { put("parserType", it) }
    }
